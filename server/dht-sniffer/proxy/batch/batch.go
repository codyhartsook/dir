...
func (hbh *Batch) Put(ctx context.Context, key datastore.Key, value []byte) error {
	if hbh.options.BeforePut != nil {
		key, value = hbh.options.BeforePut(key, value)
	}
	err := hbh.bch.Put(ctx, key, value)
...
func (hbh *Batch) Delete(ctx context.Context, key datastore.Key) error {
	if hbh.options.BeforeDelete != nil {
		key = hbh.options.BeforeDelete(key)
	}
	err := hbh.bch.Delete(ctx, key)
...
func (hbh *Batch) Commit(ctx context.Context) error {
	if hbh.options.BeforeCommit != nil {
		hbh.options.BeforeCommit()
	}
	err := hbh.bch.Commit(ctx)

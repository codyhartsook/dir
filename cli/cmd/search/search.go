...
2. List published records on the local node:
	...
	dirctl search "/skills/Text Completion"

3. List published records across the whole network:

	dirctl search "/skills/Text Completion" --network
...
func runCommand(cmd *cobra.Command, labels []string) error {
	...
	if opts.Network {
		return searchNetwork(cmd, client, labels)
	}

	return searchPeer(cmd, client, opts.PeerID, labels)
}

func init() {
	// Common flags for all search subcommands
